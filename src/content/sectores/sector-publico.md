---
titulo: Sector público
keywordPrimaria: servicios generales para entidades del Estado
descripcionSEO: "Servicios generales para entidades del Estado en el Perú: limpieza, mantenimiento e instalaciones con RNP, homologación de proveedores y cumplimiento de SST."
resumen: >-
  Servicios generales para entidades del Estado: limpieza, mantenimiento e
  instalaciones con respaldo documentario, RNP y cumplimiento de seguridad y
  salud en el trabajo (SST).
retos:
  - Cumplimiento de las bases y términos de referencia de cada proceso
  - Sustento documentario y trazabilidad del servicio
  - Continuidad de la atención al ciudadano
  - Cumplimiento estricto de seguridad y salud en el trabajo (SST)
serviciosRelacionados:
  - limpieza-empresarial
  - mantenimiento-inmuebles
  - instalaciones
  - areas-verdes
imagen:
  src: ../../assets/sectores/sector-publico.png
  alt: Edificación de una entidad del Estado atendida por FEGHADAL
orden: 4
---

## Servicios generales para entidades del Estado

FEGHADAL está preparada para atender **servicios generales para entidades del Estado**
peruano, cumpliendo las bases de cada proceso con personal capacitado, supervisión y
respaldo documentario.

### Credenciales para licitar

- **Registro Nacional de Proveedores (RNP)** vigente para contratar con el Estado.
- Disposición para procesos de **homologación de proveedores** y acreditación de
  experiencia.
- Cumplimiento de **seguridad y salud en el trabajo (SST)** y entrega de la
  documentación que exigen las entidades.

### Retos del sector público

Las entidades requieren continuidad en la atención al ciudadano, sustento de cada
actividad y estricto cumplimiento normativo. Diseñamos el servicio para responder a
los términos de referencia y a la fiscalización propia de la contratación pública.

### Por qué elegir a FEGHADAL

Integramos limpieza, mantenimiento e instalaciones bajo un mismo proveedor, con
trazabilidad y reportes, facilitando la gestión y el cumplimiento contractual en el
marco de las contrataciones del Estado.
